...
export function render(d, cs, ts, hs) {
  ...
  <div style="min-height:100vh;">
    <header style="padding:clamp(3rem,10vw,6rem) clamp(1.5rem,5vw,4rem);max-width:1100px;margin:0 auto;">
      ...
      <h1 style="${headingStyle(hs, 'font-size:clamp(3rem,8vw,6rem);margin:0;color:var(--fg);font-weight:700;line-height:1.05;letter-spacing:-0.02em;')}">${d.name}</h1>
    ...
    <section style="padding:0 clamp(1.5rem,5vw,4rem) 4rem;max-width:1100px;margin:0 auto;">
      ...
      ${d.projects.map((p, i) => `
        <div style="display:flex;gap:2rem;align-items:flex-start;padding:2rem 0;${i < d.projects.length - 1 ? 'border-bottom:1px solid var(--border);' : ''}" class="mag-project">
          <style>@media(max-width:640px){ .mag-project { flex-direction: column !important; } .mag-project > div:first-child { width: 100% !important; } }</style>
    ...
    <footer style="padding:2rem clamp(1.5rem,5vw,4rem);border-top:1px solid var(--border);max-width:1100px;margin:0 auto;">
