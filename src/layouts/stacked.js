...
export function render(d, cs, ts, hs) {
  const uid = 'sk' + Math.random().toString(36).slice(2, 6);
  return `
  <style>
    .${uid}-about { display: flex; flex-wrap: wrap; gap: 3rem; align-items: center; }
    .${uid}-about > div:first-child { flex: 1; min-width: 0; }
    .${uid}-about > div:last-child { flex-shrink: 0; max-width: 100%; }
    .${uid}-timeline { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 1rem; }
    .${uid}-row { display: grid; grid-template-columns: auto 1fr; gap: 2rem; padding: 2rem 0; }
    .${uid}-num { font-family: monospace; font-size: 2.5rem; font-weight: 700; color: color-mix(in srgb, var(--accent) 25%, transparent); line-height: 1; padding-top: 0.25rem; }
    @media (max-width: 640px) {
      .${uid}-about { gap: 1.5rem; }
      .${uid}-row { grid-template-columns: 1fr !important; gap: 0.5rem !important; }
      .${uid}-num { display: none; }
      .${uid}-timeline { grid-template-columns: 1fr 1fr; }
    }
  ...
  <div style="min-height:100vh;">
    <nav style="max-width:1000px;margin:0 auto;padding:1.5rem clamp(1rem,4vw,3rem);display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;">
    ...
    <section style="background:var(--bg2);padding:clamp(2rem,8vw,6rem) clamp(1rem,4vw,3rem);margin-top:1rem;">
      <div class="${uid}-about" style="max-width:1000px;margin:0 auto;">
        <div>
          <p style="font-family:monospace;font-size:0.8rem;color:var(--accent);margin:0 0 1rem;">// about</p>
          <h1 style="${headingStyle(hs, 'font-size:clamp(1.75rem,5vw,3rem);margin:0;color:var(--fg);font-weight:700;line-height:1.15;')}">${d.role}</h1>
    ...
    <section style="padding:clamp(2rem,6vw,4rem) clamp(1rem,4vw,3rem);">
      <div style="max-width:1000px;margin:0 auto;">
        ...
        <div class="${uid}-timeline">
          ${d.timeline.map(t => `
            <div style="padding:1rem;background:var(--card);border:1px solid var(--border);border-radius:8px;">
    ...
    <section style="padding:clamp(2rem,8vw,5rem) clamp(1rem,4vw,3rem);">
      <div style="max-width:1000px;margin:0 auto;">
        ...
        <div style="display:flex;flex-direction:column;gap:0;">
          ${d.projects.map((p, i) => `
            <div class="${uid}-row" style="${i < d.projects.length - 1 ? 'border-bottom:1px solid var(--border);' : ''}">
              <div class="${uid}-num">0${i + 1}</div>
              <div>
    ...
    <footer style="background:var(--bg2);padding:2rem clamp(1rem,4vw,3rem);">
